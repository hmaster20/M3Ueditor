    ...
    public class IPTextBox : System.Windows.Forms.UserControl
    {
        ...
        public override string Text
        {
            ...
            set
            {
                if (value != "" && value != null)
                {
                    string[] pieces = value.ToString().Split(".".ToCharArray(), 4);
                    Box1.Text = pieces[0];
                    Box2.Text = pieces[1];
                    Box3.Text = pieces[2];
                    Box4.Text = pieces[3];
        ...
        /// <summary>Returns whether all Box1 thru Box4 have a valid IP octet return True if valid, false otherwise</summary>
        public bool IsValid()
        {
            try
            {
            ...
            if (Char.IsDigit(e.KeyChar) || e.KeyChar == 8)
            {
                if (Box4.SelectionLength != Box4.Text.Length)
                {
                    if (Box4.Text.Length == 2)
                    {
                        if (e.KeyChar == 8)
                        {
                            Box4.Text = Box4.Text.Remove(Box4.Text.Length - 1, 1);
                        }
